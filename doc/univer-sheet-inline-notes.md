...
- 上层使用 Editor.js 时，在初始化或运行时开启 `readOnly` 模式：
  - 例如：`new EditorJS({ ..., readOnly: true })`；
  - 或通过 `editor.readOnly.toggle(true)` 动态切换。
- 预期：
  - 所有 BlockTool 都进入只读展示状态，不再允许用户编辑内容。
- 实际现象：
  - `editorjs-univer` 的 `univerSheet` 块在 Editor.js 只读模式下仍然可以点击表格、编辑单元格。
- 原因分析（结合现有实现）：
  - BlockTool 构造函数中已经接收了 `readOnly` 参数并保存到 `this.readOnly`；
  - 但在 `render()` 和后续挂载 Univer 的流程中，并**没有根据 `this.readOnly` 去限制交互**或调整 UI；
  - Univer 本身在当前使用方式下也没有单独的「只读模式」开关被配置。

#### 6.2 处理思路

- 在不侵入 Univer 内部实现的前提下，优先采用**外层容器控制交互**的方式：
  - Editor.js 的只读状态在 BlockTool 级别已经可见（构造参数 `readOnly`）；
  - 利用这个标记，在 Block 的根节点上加入一个只读态的 class；
  - 然后用 CSS 禁用内嵌表格区域的鼠标交互（`pointer-events: none`），从而阻止编辑。
- 这种方式的特点：
  - 实现简单、对 Univer 版本和内部 API 依赖较少；
  - 只影响交互，不改变渲染内容，适合只读展示场景。

#### 6.3 具体改动

1. **渲染阶段根据 `readOnly` 打标记 class**

   在 `UniverSheetTool.render()` 中，如果当前 Block 处于只读模式，则在外层 `wrapper` 上添加只读类名：

   - 关键代码（简化示意）：
     - 判断：`if (this.readOnly) { wrapper.classList.add('cdx-univer-sheet--readonly'); }`
   - 含义：
     - 该类名仅表示「当前 Editor.js Block 处于只读模式」，不直接参与功能逻辑，只作为 CSS 选择器入口。

2. **CSS 中禁用内嵌容器的鼠标交互**

   在 `index.css` 中追加样式规则：

   - 选择器：`.cdx-univer-sheet--readonly .cdx-univer-sheet__inline-container`
   - 核心属性：
     - `pointer-events: none;`
   - 效果：
     - 当 Editor.js 只读时，内嵌的 Univer 表格区域不再响应鼠标点击/拖拽等操作；
     - 视觉上表格仍完整展示，可随页面整体滚动查看，只是无法进行编辑。

#### 6.4 使用与注意事项

- **使用方式**（对接 Editor.js）：
  - 只需按 Editor.js 常规方式开启只读即可，例如：
    - 初始化时：`new EditorJS({ ..., readOnly: true })`；
    - 运行时：`editor.readOnly.toggle(true)`。
  - 当 Editor.js 以只读方式渲染 `univerSheet` 块时：
    - BlockTool 构造函数会接收到 `readOnly: true`；
    - `render()` 会给外层 DOM 加上 `cdx-univer-sheet--readonly`；
    - CSS 随之禁用内嵌表格的交互。

- **动态切换的说明**：
  - Editor.js 在切换 readOnly 模式时，会重新处理 BlockTool 的生命周期（取决于具体版本与配置）；
  - 在常规用法下，当只读模式生效时，新渲染出来的 `univerSheet` Block 会自动带上只读 class，内嵌表格交互被禁用。

- **后续可能的增强方向**：
  - 如果未来需要「可选中但不可编辑」等更细粒度的只读行为，可以考虑：
    - 调研 Univer 是否提供内建的只读/保护工作表能力；
    - 在 `mountUniver()` 中根据 `this.readOnly` 配置对应的只读选项；
    - 结合当前外层 CSS 控制，形成更精细的控制策略。
